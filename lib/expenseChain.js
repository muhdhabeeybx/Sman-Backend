 ...
 *
 *   · A paid expense is closed. The money has left the bank, so the figures
 *     are now a record of what happened rather than a proposal, and changing
 *     them would silently restate a settled payment. Nobody edits it — not
 *     the submitter, not an admin, not a super admin.
 ...
 * the right HTTP code and the exact reason.
 *
...
function canEditExpense(expense, user) {
  ...
  if (status === STATUS.PAID) {
    return {
      ok: false,
      status: 409,
      message: "This expense is paid and closed — its details can no longer be edited.",
    };
  }
