...
function computeFinancials(pfi, agg = {}) {
   ...
   *
   * It divides `totalCost` (cargo value + paid expenses), not the
   * credit-adjusted `grandTotalCost` it used to. The report prints "Total
   * Cost" and "Landing Cost" one line apart, and dividing a different number
   * than the one shown above meant the two could not be reconciled by anyone
   * reading it: on a batch carrying a credit note the arithmetic simply did
   * not come out, and there was no way to tell that from the page.
   *
   * Nothing here is stored, so this moves the instant any input moves — add an
   * expense, correct the unit price, fix the BL figure, and the landing cost is
   * already right the next time it is read. That was true before and is worth
   * keeping true.
   *
  ...
  const landingCostPerLitre =
    totalCost != null && costQty > 0 ? money(totalCost / costQty) : null;

  /**
   * The same figure after a credit note is taken off.
   *
   * Kept because it is the truer picture of what the cargo cost once a rebate
   * or claim is settled — but it is NOT the headline number, because the
   * headline has to divide out against the Total Cost printed beside it.
   * Identical to landingCostPerLitre whenever there is no credit, which is
   * most batches.
   */
  const landingCostPerLitreAfterCredit =
  ...
  // promised, and the gap between the two IS the cost of the deficit.
  const landingCostPerLitreTank =
    totalCost != null && tankQty > 0 ? money(totalCost / tankQty) : null;
  ...
  return {
    ...
    grandTotalCost,
    landingCostPerLitre,
    landingCostPerLitreAfterCredit,
...
function explainFinancials(pfi, f) {
  ...
  return [
      ...
      : []),
    {
      ...
      meaning:
        "Everything this batch has cost so far — the product plus every paid expense against it.",
    },
    {
      ...
      workings: `${NGN(f.totalCost)} ÷ ${QTY(f.costQtyLitres, unit)} = ${NGN(f.landingCostPerLitre)}`,
      meaning: `What one ${one.toLowerCase()} of this batch cost you, all in. Sell below this and the batch loses money. It recalculates automatically — add an expense or correct the price and this figure moves with it.`,
    },
    ...(f.creditBalance > 0
      ? [
          {
          ...
          },
          {
            ...
            formula: "Total Cost − Credit Note",
            workings: `${NGN(f.totalCost)} − ${NGN(f.creditBalance)} = ${NGN(f.grandTotalCost)}`,
            meaning:
              "What the batch cost after the credit is taken off. This is what profit is measured against.",
          },
          {
            key: "landingCostPerLitreAfterCredit",
            label: `Landing Cost per ${one} (after credit)`,
            value: NGN(f.landingCostPerLitreAfterCredit),
            formula: `Grand Total Cost ÷ ${billedLabel}`,
            workings: `${NGN(f.grandTotalCost)} ÷ ${QTY(f.costQtyLitres, unit)} = ${NGN(f.landingCostPerLitreAfterCredit)}`,
            meaning:
              "The truer cost per unit once the credit is settled. Shown only because this batch carries a credit note.",
          },
        ]
      : []),
      ...
      : [
          {
            ...
            workings: `${NGN(f.totalCost)} ÷ ${QTY(f.tankQtyLitres, unit)} = ${NGN(f.landingCostPerLitreTank)}`,
            meaning:
